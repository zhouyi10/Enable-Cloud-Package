(function($,window) {
	var ConnectLineQuestion = window.ConnectLineQuestion = {
		itemTemplate : "<span class='showitem' data-id='{0}' order='{1}'><img style='' src='{2}'/></span>",
		init : function(){
			$(".question_area .demo").onLine({
				regainCanvas: true
			});
		},
		initEvent : function(){
		...
		getUserAnswer : function(settings){
			...
			var answerArr = [];
			var ret = false;
			$this.find(".showright .showitem").each(function(){
				if ($(this).hasClass("addstyle")){
					var pair = $(this).attr("pair");
					answerArr.push($this.find(".showleft .showitem[pair='"+pair+"']").attr("order"));
					ret = true;
				}else{
					answerArr.push(" ");
				}
	...
	$.fn.onLine = function(options){
		var box = this;
		$(".show .showleft img, .show .showright img").on('mousedown',function (e) {
			e.preventDefault()
		});
		...
		$(".collectline").mousemove(function(event){		//移动
		...
		$(".collectline").mouseup(function(event){  //抬起
			var $target = $(event.target);
			if(groupstate){
				var targettrue;
				if($target.is("span") && $target.closest(".show").parent().attr("class")==box.attr("class") ){
		...
		function clearline(){//清除
			...
			part2.attr('first',0);
			if (pair == 0){
				$("#testCard li.active_li").find("div").removeClass("active_li");
			}
		};
