    ...
    var perView = win.PerView = {
        ...
        hiddenAnswer: function () {
            ...
            $("#exportPdf").on("click", function(){
                if (_this.paperInfo.paperId!=null&&_this.paperInfo.paperId.trim()!="") {
                    var url = location.host+CommUtils.formatStr( PerView.viewPaperUrl, _this.paperInfo.paperId)+ "?printPdf=true&window-status=completed";
                    window.open(_this.expertPDFUrl + window.btoa(url),'_blank', '');
                }else {
                    layer.msg('请先保存试卷！', { icon: 7, time: 1000, shade: [0.6, '#000', true] });
                }
        ...
        default: function () {
            ...
            if (_this.examStypeinfoPO.recordScoreColumn == 1){
                var bigtopic = _this.paperInfo.sumBigtopic;
